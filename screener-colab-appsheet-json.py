...
import math, json, re, time, io, warnings, logging, os, sys
...
LOCAL_JSON_FILE = "gas_stock_tracker.json"
LOCAL_DASHBOARD_JSON_FILE = "gas_stock_tracker_dashboard.json"

# Backward-compatible internal names. The JSON clone keeps the original
# workbook-based calculation pipeline in memory, but only persists JSON.
S3_EXCEL_KEY = S3_JSON_KEY
S3_DASHBOARD_EXCEL_KEY = S3_DASHBOARD_JSON_KEY
LOCAL_EXCEL_FILE = LOCAL_JSON_FILE
LOCAL_DASHBOARD_EXCEL_FILE = LOCAL_DASHBOARD_JSON_FILE
...
YF_DELAY_SEC          = 0.35
YF_CACHE_MAX          = 300
OFF_MARKET_SIGNAL_SNAPSHOT_ENABLED = False  # Disabled: always fetch fresh from screener+YF on each Colab session; Days Tracked persists via S3 JSON
...
RELOAD_WB_EVERY           = 3      # reload JSON-backed in-memory workbook from S3 every N iterations (prevents RAM bloat)
...
DASHBOARD_DB_ENABLED      = True     # append Dashboard rows to JSON each iteration
DASHBOARD_DB_FILE         = "dashboard_snapshots.json"
...
JSON_FORMAT_VERSION = "colab-screener-json-v1"
...
def _worksheet_to_json(ws):
    max_row = ws.max_row or 0
    max_col = ws.max_column or 0
    headers = []
    for ci in range(1, max_col + 1):
        header = ws.cell(row=1, column=ci).value
        headers.append(str(header).strip() if header is not None else f"Column {ci}")
    ...
    for ri in range(2, max_row + 1):
        ...
        for ci, header in enumerate(headers, 1):
            value = _json_safe_value(ws.cell(row=ri, column=ci).value)
...
def _workbook_to_json_payload(wb, sheet_names=None):
    selected_sheets = list(sheet_names) if sheet_names else list(wb.sheetnames)
    return {
        "format_version": JSON_FORMAT_VERSION,
        "generated_at": fmt_dt(ist_now()),
        "sheet_order": selected_sheets,
        "sheets": {
            name: _worksheet_to_json(wb[name])
            for name in selected_sheets
            if name in wb.sheetnames
        },
    }

def _json_payload_to_workbook(payload):
    wb = openpyxl.Workbook()
    if "Sheet" in wb.sheetnames:
        del wb["Sheet"]

    sheets = payload.get("sheets", {}) if isinstance(payload, dict) else {}
    sheet_order = payload.get("sheet_order") if isinstance(payload, dict) else None
    ordered_names = sheet_order if isinstance(sheet_order, list) else list(sheets.keys())
...
def _serialize_workbook_json_bytes(wb, sheet_names=None) -> bytes:
    payload = _workbook_to_json_payload(wb, sheet_names=sheet_names)
    return json.dumps(payload, ensure_ascii=False, indent=2).encode("utf-8")

def _workbook_from_json_bytes(data: bytes):
    payload = json.loads(data.decode("utf-8"))
    return _json_payload_to_workbook(payload)

def s3_download_excel(retries=3):
    """Download JSON state from S3 with retry logic.
    Returns:
      - in-memory workbook reconstructed from JSON on success
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            data = obj["Body"].read()
            wb   = _workbook_from_json_bytes(data)
            print(f"  [DL] Downloaded existing JSON from S3 ({len(data)//1024} KB)")
            return wb
        except ClientError as e:
            code = e.response.get("Error",{}).get("Code","")
            if code in ("NoSuchKey","404"):
                print("  📂 No existing JSON on S3 -- first run, creating fresh in-memory workbook")
...
def _s3_get_existing_size():
    """Get the size in bytes of the current S3 JSON file (0 if not found)."""
    try:
...
def s3_backup_before_upload():
    """Archive current S3 JSON to a timestamped backup key before overwriting."""
    ts = ist_now().strftime("%Y-%m-%d_%H%M%S")
    backup_key = f"reports/backups/gas_stock_tracker_{ts}.json"
    try:
        _s3().copy_object(
            Bucket=S3_BUCKET,
...
def s3_upload_excel(wb, save_local: bool = True, backup: bool = True):
    try:
        data = _serialize_workbook_json_bytes(wb)
    except Exception as e:
        print(f"  [FAIL] JSON serialise failed: {e}"); return False

    if save_local:
        _save_local_artifact(data, LOCAL_EXCEL_FILE, "JSON")
    ...
    for attempt in range(1, S3_UPLOAD_RETRIES + 1):
        try:
            ...
            print(f"  📤 Uploaded JSON ({len(data)//1024} KB) -> s3://{S3_BUCKET}/{S3_EXCEL_KEY}")
            return True
        except Exception as e:
...
def s3_upload_dashboard_excel(wb):
    """Build, save, and upload dashboard-only JSON."""
    try:
        print("  [STAT] Building dashboard-only JSON...")
        data = _serialize_workbook_json_bytes(wb, sheet_names=["Dashboard"])
        print(f"  [STAT] Dashboard-only JSON ready ({len(data)//1024} KB)")
    except Exception as e:
        print(f"  [FAIL] Dashboard JSON serialise failed: {e}")
        return False

    _save_local_artifact(data, LOCAL_DASHBOARD_EXCEL_FILE, "Dashboard JSON")
    ...
    for attempt in range(1, S3_UPLOAD_RETRIES + 1):
        try:
            ...
            print(f"  📤 Uploaded Dashboard JSON ({len(data)//1024} KB) -> s3://{S3_BUCKET}/{S3_DASHBOARD_EXCEL_KEY}")
            return True
        except Exception as e:
            print(f"  [WARN]  Dashboard JSON upload attempt {attempt}/{S3_UPLOAD_RETRIES} failed: {e}")
            if attempt < S3_UPLOAD_RETRIES:
                time.sleep(5 * attempt)
    print("  [FAIL] Dashboard JSON upload gave up after all retries.")
    return False
...
def _empty_dashboard_snapshots_payload():
    return {
        "format_version": "colab-screener-dashboard-snapshots-json-v1",
        "table": DASHBOARD_DB_TABLE,
        "updated_at": None,
        "snapshots": [],
    }

def _load_dashboard_snapshots_payload(path: str):
    if not path or not os.path.exists(path):
        return _empty_dashboard_snapshots_payload()
    try:
        with open(path, "r", encoding="utf-8") as f:
            payload = json.load(f)
        if isinstance(payload, dict) and isinstance(payload.get("snapshots"), list):
            payload.setdefault("format_version", "colab-screener-dashboard-snapshots-json-v1")
            payload.setdefault("table", DASHBOARD_DB_TABLE)
            return payload
        if isinstance(payload, list):
            wrapped = _empty_dashboard_snapshots_payload()
            wrapped["snapshots"] = payload
            return wrapped
    except Exception as e:
        print(f"  [WARN]  Dashboard snapshots JSON load failed, starting a fresh JSON payload: {e}")
    return _empty_dashboard_snapshots_payload()

def _dashboard_snapshot_record(row: dict, snap_ts: str, iteration: int, row_no: int) -> dict:
    ...
    return {
        ...
        "name": name or None,
        "in_screener": row.get("In Screener?"),
        "quick_action": row.get("Quick Action"),
        "consensus_score": _as_float(row.get("Consensus Score")),
        "mtf_alignment": row.get("MTF Alignment"),
        "historical_mtf": row.get("Historical MTF"),
        "sector": row.get("Sector"),
        "industry": row.get("Industry"),
        "sector_benchmark": row.get("Sector Benchmark"),
        "rs_tag": row.get("RS Tag"),
        "rs_vs_nifty_1m": _as_float(row.get("RS vs NIFTY 1M%")),
        "rs_vs_nifty_3m": _as_float(row.get("RS vs NIFTY 3M%")),
        "rs_vs_sector_1m": _as_float(row.get("RS vs Sector 1M%")),
        "rs_vs_sector_3m": _as_float(row.get("RS vs Sector 3M%")),
        "avg_traded_value_20d_cr": _as_float(row.get("Avg Traded Value 20D Cr")),
        "liquidity_tag": row.get("Liquidity Tag"),
        "momentum_rank": _as_int(row.get("Momentum Rank")),
        "momentum_tag": row.get("Momentum Tag"),
        "risk_tag": row.get("Risk Tag"),
        "bb_signal": row.get("BB Signal"),
        "cam_setup": row.get("Cam Setup"),
        "cam_h3": _as_float(row.get("Cam H3")),
        "cam_h4": _as_float(row.get("Cam H4")),
        "cam_l3": _as_float(row.get("Cam L3")),
        "cam_l4": _as_float(row.get("Cam L4")),
        "ideal_enter_price": _as_float(row.get("Ideal Enter Price")),
        "possible_sell_value": _as_float(row.get("Possible Sell Value")),
        "stop_loss_value": _as_float(row.get("Stop Loss Value")),
        "volume_buzz": row.get("Volume Buzz"),
        "since_capture_trend": row.get("Since Capture Trend"),
        "signal": row.get("Signal"),
        "setup_signal": row.get("Setup Signal"),
        "core_signal": row.get("Core Signal"),
        "signal_quality": row.get("Signal Quality"),
        "signal_regime": row.get("Signal Regime"),
        "win_prob": _as_float(row.get("Win Prob%")),
        "hist_precision": _as_float(row.get("Hist Precision%")),
        "exp_5d": _as_float(row.get("Exp 5D%")),
        "exp_10d": _as_float(row.get("Exp 10D%")),
        "wf_samples": _as_int(row.get("WF Samples")),
        "ai_decision": row.get("AI Decision"),
        "ai_conf": _as_float(row.get("AI Conf%")),
        "payload": row,
    }

def append_dashboard_snapshot_to_db(wb, iteration: int, snapshot_at: str | None = None):
    """Append current Dashboard sheet rows into local JSON snapshot history."""
    dash_ws = ensure_dashboard_sheet(wb)
    json_path = _dashboard_db_local_path()
    json_dir = os.path.dirname(json_path)
    if json_dir:
        os.makedirs(json_dir, exist_ok=True)

    snap_ts = snapshot_at or fmt_dt()
    payload = _load_dashboard_snapshots_payload(json_path)
    payload["updated_at"] = snap_ts
    ...
    for ri in range(2, dash_ws.max_row + 1):
        ...
        payload["snapshots"].append(_dashboard_snapshot_record(row, snap_ts, iteration, ri))
        inserted += 1

    with open(json_path, "w", encoding="utf-8") as f:
        json.dump(payload, f, ensure_ascii=False, indent=2)

    return inserted, json_path
...
def _s3_backup_key_before_upload(key: str):
    ...
    try:
        ...
        print(f"  🗂️  Backup -> s3://{S3_BUCKET}/{backup_key}")
        return True
    except ClientError as e:
        code = e.response.get("Error",{}).get("Code","")
        if code in ("NoSuchKey","404"):
            return True
        print(f"  [WARN]  Backup failed [{code}]: {e}")
        return False
    except Exception as e:
        print(f"  [WARN]  Backup failed: {e}")
        return False
...
def s3_upload_dashboard_db(db_path: str):
    """Upload local dashboard snapshots JSON to S3 with retry + backup."""
    if not db_path or not os.path.exists(db_path):
        print(f"  [WARN]  Dashboard snapshots JSON not found: {db_path}")
        return False
    ...
    try:
        ...
        print(f"  [FAIL] Dashboard snapshots JSON read failed: {e}")
        return False

    if not data:
        print("  [WARN]  Dashboard snapshots JSON is empty -- skipping S3 upload.")
        return False
    ...
    for attempt in range(1, S3_UPLOAD_RETRIES + 1):
        try:
            ...
            print(f"  📤 Uploaded Dashboard snapshots JSON ({len(data)//1024} KB) -> s3://{S3_BUCKET}/{S3_DASHBOARD_DB_KEY}")
            return True
        except Exception as e:
            print(f"  [WARN]  Dashboard snapshots JSON upload attempt {attempt}/{S3_UPLOAD_RETRIES} failed: {e}")
            if attempt < S3_UPLOAD_RETRIES:
                time.sleep(5 * attempt)
    print("  [FAIL] Dashboard snapshots JSON upload gave up after all retries.")
    return False
...
def init_workbook():
    ...
    if result is None:
        # Transient S3 failure -- DO NOT create fresh state (would wipe data)
        raise RuntimeError(
            "[FAIL] FATAL: Cannot download JSON state from S3 after 3 retries. "
            "Refusing to start with empty state to prevent data loss. "
            "Check S3 connectivity and try again."
        )
    if result == _S3_FIRST_RUN:
        # Genuinely first run -- create fresh in-memory workbook
        print("  📂 Creating brand new JSON-backed workbook state (first run)")
        wb = openpyxl.Workbook()
...
def tg_send_document(path, caption=""):
    if not TELEGRAM_BOT_TOKEN or not TELEGRAM_CHAT_ID:
        return False
    if not path or not os.path.exists(path):
        print(f"  [WARN] Telegram document missing: {path}")
        return False
...
def main(max_iterations=None):
    ...
    start_msg = (f"[START] Screener Tracker Started (PARALLEL ×{PARALLEL_WORKERS})\n{fmt_dt(ist_now())}\n"
                 ...
                 f"S3 JSON: s3://{S3_BUCKET}/{S3_EXCEL_KEY}\n"
                 f"S3 Dashboard JSON: s3://{S3_BUCKET}/{S3_DASHBOARD_EXCEL_KEY}")
    if DASHBOARD_DB_ENABLED:
        start_msg += f"\nS3 Dashboard Snapshots JSON: s3://{S3_BUCKET}/{S3_DASHBOARD_DB_KEY}"
    tg_send(start_msg)
    iteration = 0
    try:
        while True:
            ...
            print(f"{'='*62}")
            try:
                # Periodic JSON-backed workbook reload: flush accumulated RAM
                if iteration > 1 and (iteration - 1) % RELOAD_WB_EVERY == 0:
                    print("  [RELOAD] Reloading JSON state from S3 (periodic memory flush)...")
                    ...
                    if wb_fresh and wb_fresh != _S3_FIRST_RUN:
                        ...
                        ensure_validation_sheet(wb)
                        for sc in SCANNERS: ensure_scanner_sheet(wb, sc)
                        print("  [OK] JSON state reloaded.")
                    else:
                        print("  [WARN]  Reload failed -- continuing with existing in-memory state.")
                ...
                # -- Append Dashboard snapshots JSON + upload -------------
                db_ok = None
                db_rows = 0
                if DASHBOARD_DB_ENABLED and dashboard_updated:
                    try:
                        ...
                        print(f"  [SNAP] Appended {db_rows} dashboard rows -> {db_path}")
                        db_ok = s3_upload_dashboard_db(db_path)
                    except Exception as e:
                        db_ok = False
                        print(f"  [WARN]  Dashboard snapshots JSON append/upload failed: {e}")
                elif DASHBOARD_DB_ENABLED:
                    db_ok = False
                    print("  [WARN]  Skipping Dashboard snapshots JSON append because Dashboard update failed.")
                ...
                lines = [f"[OK] Iteration #{iteration} done (PARALLEL ×{PARALLEL_WORKERS})",
                         ...
                         f"📁 JSON {'saved [OK]' if ok else 'FAILED [FAIL]'}",
                         f"📊 Dashboard JSON {'saved [OK]' if dash_excel_ok else 'FAILED [FAIL]'}",
                         f"🆕 New: {len(all_new)}"]
                ...
                if DASHBOARD_DB_ENABLED:
                    lines.append(f"[SNAP] JSON {'saved [OK]' if db_ok else 'FAILED [FAIL]'} ({db_rows} rows)")
                if all_new: lines += [f"  - [{sc}] {nm}" for sc, nm in all_new[:10]]
                if presigned_url:
                    lines.append(f"[DL] JSON: {presigned_url}")
                if dashboard_presigned_url:
                    lines.append(f"[DL] Dashboard JSON: {dashboard_presigned_url}")
                tg_send("\n".join(lines))
                if ok:
                    tg_send_document(
                        _local_output_path(LOCAL_EXCEL_FILE),
                        f"Full JSON iteration #{iteration} | {fmt_dt(ist_now())}",
                    )
                if dash_excel_ok:
                    tg_send_document(
                        _local_output_path(LOCAL_DASHBOARD_EXCEL_FILE),
                        f"Dashboard JSON iteration #{iteration} | {fmt_dt(ist_now())}",
                    )
                ...
                status_line = f"\n  JSON: {'[OK]' if ok else '[FAIL]'}  |  Dashboard JSON: {'[OK]' if dash_excel_ok else '[FAIL]'}"
                if validation_result:
                    status_line += (
                        f"  |  VALID: {validation_result.get('Status')} "
                ...
                if DASHBOARD_DB_ENABLED:
                    status_line += f"  |  SNAP JSON: {'[OK]' if db_ok else '[FAIL]'} ({db_rows} rows)"
                status_line += f"  |  New: {len(all_new)}  |  [TIME] {elapsed}s"
                print(status_line)
                if presigned_url:
                    print(f"  [LINK] JSON presigned URL generated (24h expiry)")
                if dashboard_presigned_url:
                    print(f"  [LINK] Dashboard JSON presigned URL generated (24h expiry)")
                ...
                print("  [SAVE] Emergency S3 JSON upload...")
        ...
        tg_send("[STOP] Tracker manually stopped. Saving final JSON state to S3...")
        try: s3_upload_excel(wb)
